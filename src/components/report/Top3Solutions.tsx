...
export default function Top3Solutions({ recommendations, language = 'EN', goals = [], onUnlock, selectedProtocolId, onSelectProtocol }: Top3SolutionsProps) {
    const [expandedRank, setExpandedRank] = useState<number | null>(1); // #1 open by default
    const lbl = L[language] || L['EN'];

    if (!recommendations || recommendations.length === 0) return null;

    const toggleExpand = (rank: number, protocol: Protocol) => {
        const isExpanding = expandedRank !== rank;
        setExpandedRank(isExpanding ? rank : null);
        if (isExpanding && onSelectProtocol) {
                    ...
                    return (
                        ...
                        >
                            ...
                            <button
                                ...
                                onClick={() => toggleExpand(proto.rank, proto)}
                            >
