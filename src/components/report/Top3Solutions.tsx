                                        ...
                                        {proto.doctor ? (
                                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center text-xs font-bold text-white shadow-lg border border-cyan-300/30">
                                                        {proto.doctor?.name?.[0] || 'M'}
                                                    </div>
                                                    <div>
                                                        <span className="block text-[11px] font-bold text-white font-mono leading-none">
                                                ...
                                                <button
                                                    onClick={() => {
                                                        // Fallback to Tally booking form or direct clinic link if available
                                                        const bookingUrl = proto.doctor?.booking_url || `https://tally.so/r/n0q1L9?doctor=${encodeURIComponent(proto.doctor?.name || '')}&protocol=${encodeURIComponent(proto.name)}`;
                                                        window.open(bookingUrl, '_blank');
                                                    }}
                                                    className="flex-1 sm:flex-none text-[10px] font-bold px-4 py-2 rounded-lg bg-[#00FF88]/10 text-[#00FF88] border border-[#00FF88]/30 hover:bg-[#00FF88]/20 transition-all flex items-center justify-center gap-2 group/book"
                                                >
                                                    <Activity className="w-3.5 h-3.5 group-hover/book:animate-pulse" />
                                                    Book Appointment
                                                    <ArrowUpRight className="w-3 h-3 translate-x-0 transition-transform group-hover/book:translate-x-0.5" />
                                                </button>
                                            </div>
                                        ) : (
