...
import { Loader2, Plus, Stethoscope, Users, TrendingUp, Award, Trophy, Star, Eye, Heart, CheckCircle, FileText } from 'lucide-react';
...
export default function DoctorDashboard() {
    ...
    const fetchConsultations = async () => {
        try {
            const res = await fetch(`/api/doctor/consultations?email=${encodeURIComponent(user!.email)}`);
            const data = await res.json();
            if (data.consultations) {
                setConsultations(data.consultations.slice(0, 5)); // Show latest 5
            }
        } catch (e) {
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-[#111] to-black border border-white/10 rounded-3xl p-6">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="font-bold text-white">Recent Consultations</h3>
                        </div>
                        {loadingConsultations ? (
                            ...
                            <div className="space-y-3">
                                {consultations.map(c => (
                                    <button
                                        key={c.id}
                                        onClick={() => router.push(`/doctor/patient/${c.report_id}`)}
                                        className="w-full text-left p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-cyan-500/30 transition-all"
                                    >
                                        <div className="flex items-center justify-between mb-1">
                                            <span className="text-sm font-bold text-white truncate">{c.aesthetic_goal || 'Consultation'}</span>
                                            <span className={`text-xs px-2 py-0.5 rounded-full ${c.status === 'pending' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
                                                {c.status}
                                            </span>
                                        </div>
                                        <div className="text-xs text-gray-500">
                                            {c.patient_age} · {c.patient_gender} · {c.patient_country} · {new Date(c.requested_at).toLocaleDateString()}
                                        </div>
                                    </button>
                                ))}
                            </div>
